...
def get_forums(db: Session):
    forums = db.query(models.Forum).options(joinedload(models.Forum.forum_comments).joinedload(models.ForumComment.user).load_only(models.User.user_avatar), joinedload(models.Forum.forum_members).joinedload(models.ForumMember.user).load_only(models.User.user_avatar)).all()
    return forums
